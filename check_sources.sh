...
alpine_info=$(curl -s https://hub.docker.com/v2/repositories/library/alpine/tags/latest | jq -r '.images[] | select(.architecture==("amd64", "arm64")) | {architecture: .architecture, sha: .digest} | .[] ')
...
OUTPUT="$OUTPUT last_updated:$alpine_last_updated"
OUTPUT="$OUTPUT $alpine_info"
